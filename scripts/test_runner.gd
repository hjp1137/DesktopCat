...
func _init() -> void:
	...
	# 测试 3: 点击反馈与弹跳测试
	assert(cat.bounce_offset_y == 0.0, "初始弹跳偏移应当为0")
	cat._on_clicked()
	print("[PASS] 测试 3: 点击触发 _on_clicked() 成功，Tween已创建")
	...
	print("[PASS] 测试 4: 右边界碰撞转向与钳制验证成功: x=", cat.position.x, ", dir=", cat.direction)

	cat.free()
	print("========== T03 移动与点击逻辑所有单元测试全部通过 ==========")
	quit(0)
